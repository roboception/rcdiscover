  ...
  wxDialog(parent, id, "About rc_discover", pos, wxSize(420, 300), style, name)
{
  ...
  auto *copyright = new wxStaticText(panel, wxID_ANY, "Copyright 2017 Roboception GmbH.");
  ...
  auto *licenses = new wxStaticText(panel, wxID_ANY, "This product contains code from the LGPLed library wxWidgets (http://www.wxwidgets.org/).");
  licenses->Wrap(400);
  licenses_box->Add(licenses, 1, wxEXPAND | wxALL, 10);
  vbox->Add(licenses_box, 0);

  auto *lgpl_box = new wxBoxSizer(wxHORIZONTAL);
  auto *lgpl = new wxTextCtrl(panel, wxID_ANY, wxEmptyString, wxDefaultPosition, wxSize(400, -1), wxTE_MULTILINE | wxTE_READONLY);
  lgpl->SetFont(wxFont(7, wxFONTFAMILY_DEFAULT, wxFONTSTYLE_NORMAL, wxFONTWEIGHT_NORMAL));
